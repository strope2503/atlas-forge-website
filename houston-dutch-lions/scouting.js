window.opponentScoutingDB = {
  ...
  'HTX South Gold': {
    ...
    strengths: ['High scoring power (47 GF)', 'Rapid counter-attacks', 'Dual attacking midfielders'],
    weaknesses: ['High goals conceded (49 GA)', 'Over-committed fullbacks leave counter corridors', 'Aggressive offside trap'],
    ...
    targetZones: ['Deep counter channels behind fullbacks', '1v1 footraces against CBs'],
    film: [
      { title: 'Solar STX 15B vs HTX South Gold (Match Film — 36:38)', url: 'https://www.youtube.com/watch?v=N6oJxEcP0Mg', duration: '36:38' },
      { title: 'Girls Teams vs HTX South 12G Aspire (Veo Match Recording)', url: 'https://app.veo.co/matches/20250824-girls-teams-vs-htx-south-12g-aspire-6e7c3547/', duration: 'Full Veo' }
    ]
  },
  'HTX Central Gold': {
    id: '752439',
...
window.overrideOpponentFunctions = function() {
  if (typeof window.stxclBracketBTeams === 'undefined') return;

  const originalRenderOpponents = window.renderOpponents;
  
  window.renderOpponents = function() {
    const grid = document.getElementById('opponent-grid');
    if (!grid) return;
    
    let html = '';
    window.stxclBracketBTeams.forEach(t => {
      const isHDL = t.isOurTeam;
      let scoutInfo = null;
      for (const [sName, sData] of Object.entries(window.opponentScoutingDB)) {
        if (sData.id === String(t.id) || t.name.toLowerCase().includes(sName.toLowerCase()) || sName.toLowerCase().includes(t.name.toLowerCase())) {
          scoutInfo = sData;
          break;
        }
      }
  ...
  window.openOpponentModal = function(teamIdentifier) {
    if (!teamIdentifier) return;
    const term = String(teamIdentifier).trim().toLowerCase();
    
    const team = window.stxclBracketBTeams.find(t => {
      if (String(t.id) === String(teamIdentifier) || String(t.teamId) === String(teamIdentifier)) return true;
      const nameLower = t.name.toLowerCase();
      const fullLower = t.fullName.toLowerCase();
      if (term === nameLower || term === fullLower) return true;
      if (term.includes(nameLower) || nameLower.includes(term)) return true;
      if (term.includes(fullLower) || fullLower.includes(term)) return true;
      const termClean = term.replace(/[^a-z0-9]/g, '');
      const nameClean = nameLower.replace(/[^a-z0-9]/g, '');
      if (termClean.includes(nameClean) || nameClean.includes(termClean)) return true;
      return false;
    });
    ...
    let scouting = null;
    for (const [sName, sData] of Object.entries(window.opponentScoutingDB)) {
      if (sData.id === String(team.id) || team.name.toLowerCase().includes(sName.toLowerCase()) || sName.toLowerCase().includes(team.name.toLowerCase())) {
        scouting = sData;
        break;
      }
    }
        ...
        <div class="bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 p-4 rounded-2xl border border-orange-500/40 space-y-3 shadow-xl mt-3">
          <div class="flex justify-between items-center border-b border-slate-800 pb-2">
            <div class="flex items-center gap-2">
              ...
              <h4 class="text-xs font-black uppercase tracking-wider text-orange-400 flex items-center gap-1.5">
                <i data-lucide="target" class="w-3.5 h-3.5"></i> Tactical Scouting Dossier
              </h4>
            </div>
          ...
          <div class="bg-slate-950/90 p-3 rounded-xl border border-blue-900/60 space-y-2 text-xs">
            <div class="text-[10px] font-extrabold uppercase tracking-wider text-blue-400 flex items-center gap-1.5">
              <i data-lucide="crosshair" class="w-3.5 h-3.5 text-blue-400"></i> HDL Counter-Strategy
            </div>
    ...
    content.innerHTML = `
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1 mt-2">
          <!-- Club Info -->
  ...
  if (originalRenderOpponents) {
    window.renderOpponents();
  }
};

window.addEventListener('DOMContentLoaded', () => {
  if (typeof window.overrideOpponentFunctions === 'function') {
    setTimeout(window.overrideOpponentFunctions, 100);
  }
});
